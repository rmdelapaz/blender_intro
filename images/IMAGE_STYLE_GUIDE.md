...
**Status:** v6, locked. Pilot is `lesson_01_08_folder_structure.svg`. v6 folds in the still-live L07 + L08 + L09 PRODUCTION / INTEGRATION craft that v5 did not yet carry: the color-dependent render recipe (AgX to Standard view transform · F12 `render.render` not `render.opengl` · the L09Orange / L09Cyan marker values on a 0.05 world), the grid-floor-as-real-line-geometry builder, the popover-blocks-the-bridge-grab routing note, the PIL-composite-over-PNG pixel-verify rule (with the cairosvg-is-available-on-Claude's-side render-check note), the synthetic-cage / overlay-over-render pattern, the axis-tinted composite-overlay, the small-frame fs floor, the local-tempdir-screenshot-carries-overlays plus deterministic area crop, the ghost-blend luminance dependence, the edge-crease operator plus `inset_individual` supporting-loop generator, the spread-matched-threshold craft, the order-demo legibility check, the Phase 7 Mirror-modifier 5.1.1 UI notes, and the Phase 8 clean-canvas viewport-staging plus Pillow-warning-vignette-as-overlay-host pattern. v5 (carried forward) adds the Capture workflow section (add-on dependencies, BlenderMCP capture paths, Face Orientation overlay capture recipe, manual capture cadence, modal capture criterion, visual readability in SOLID shading, multi-panel composite QA), the Blender 5.1.1 op-name and behavior discoveries section, and the cross-sub-chat composite pattern bullet in Workflow conventions. v4 (carried forward) adds the overlay badge colorway split (positive green, warning red), em-dash discipline, the inline-stacked PNG annotation overlay pattern, the 800x680 viewBox variant, HTML integration depth rules for figures inside cards, and the SVG inlining checklist (color attribute strip, content re-indent, multi-line attribute collapse, comment preservation policy).
...
- **v6 (current, 2026-06-02).** Folded in the still-live L07 + L08 + L09 PRODUCTION / INTEGRATION craft that v5 did not yet carry, sourced from the Lesson 07 / 08 carry-forward flags plus the Lesson 09 Phase 1 through Phase 10 Status log. New sections, inserted between Multi-panel composite QA and the Blender 5.1.1 op-name discoveries: Color-dependent viewport renders (F12 `render.render` not `render.opengl`; AgX to Standard view transform as the real fix for washed markers; the locked L09Orange / L09Cyan marker values on a 0.05 world; phase-parity re-render note), Grid-floor as real line geometry (the `make_grid_lines` builder; helpers inlined per call), Synthetic-cage / overlay-over-render, Local-tempdir screenshot carries overlays plus deterministic area crop, Ghost-blend luminance dependence, Edge-crease operator plus `inset_individual` supporting-loop generator, Spread-matched-threshold craft, Order-demo legibility, Phase 7 Mirror-modifier and X-Mirror 5.1.1 UI notes, Phase 8 clean-canvas viewport staging plus the Pillow warning-vignette overlay-host pattern, and Pixel-verify every PNG overlay before commit (with the cairosvg-available-on-Claude's-side vs PIL-composite-substitute-bridge-side split). Locked decisions grew from 16 to 21 entries (color-dependent render recipe; grid-floor real line geometry; pixel-verify before commit; capture-path routing; warning-vignette overlay host). All additions sourced from completed L07 / L08 / L09 work; no existing SVGs or PNG overlays need re-issue. Lesson 09 image work complete (55/55) at the time of this revision.
- **v5 (2026-05-25).** Added the Capture workflow section between HTML integration patterns and Accessibility requirements, covering add-on dependencies (LoopTools v4.7.7 Extensions Platform install path), BlenderMCP capture paths (WSL UNC truncation workaround for `bpy.ops.screen.screenshot`; capture-path selection `screenshot_area` vs `render.opengl` for overlay-layer inclusion), a worked Face Orientation overlay capture recipe (theme alpha bump, overlay-aware capture path, chrome-aware Pillow crop), manual capture cadence (save direct to WSL `images/`, Win+Shift+S popover preservation on Win 11, Pillow rewrite-in-place via `blender:execute_blender_code`), the modal capture criterion (live operator readout top floating or bottom bar plus modal keybinding hints bottom bar; status bar authoritative for bindings; knife-tool 5.1.1 A/C/Z worked example), visual readability rules for SOLID shading (`show_wire` ON before `knife_project`; `Alt+Z` X-ray ON before knife Cut-Through), and multi-panel composite QA (first-pass marginal-read rejection check before the composite step). Added the Blender 5.1.1 op-name and behavior discoveries section between Capture workflow and Accessibility requirements (Screw Modifier rotation_euler caveat, subdivide VIEW_3D `temp_override` requirement and quadratic `(N+1)²` face growth, multi-object Edit Mode breaks knife_project workaround, Alt+E Extrude menu 6-item count in 5.x with Extrude Manifold added, narrower-scope view_rotation-vs-view_axis sync note). Added the cross-sub-chat composite pattern bullet to Workflow conventions (scriptable panels retained as `_temp_*.png` for the receiving sub-chat to composite with manual panel). Locked decisions grew from 13 to 16 entries (modal capture criterion; visual readability in SOLID; manual capture save-direct-to-WSL). All additions sourced from Lesson 06 Phase 2 through Phase 6 PRODUCTION discoveries; no existing SVGs or PNG overlays need re-issue. Superseded by v6.
