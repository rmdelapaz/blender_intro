...
**Status:** v8, locked. Pilot is `lesson_01_08_folder_structure.svg`. Current rules live in the body sections plus the Locked decisions list; version-by-version history is in the Revision log at the foot of this file.
...
- **Pillow can rewrite UNC-path PNGs in place via `blender:execute_blender_code`** when a user-captured snip comes in oversized. Working pattern: read the snip via `PIL.Image.open(unc_path)`, run `image.crop(bounds)` and/or `image.resize(target, Image.LANCZOS)`, save back to the same path. Verified on Lesson 06 Phase 4 #45 (3838x2160 → 1920x1080) and Phase 6 #39 (3838x2160 → 1920x848). Verify final size via `Filesystem:get_file_info` against the accept-size band for the figure's class (see PNG accept-size band below) before integration.

### PNG accept-size band (by figure class, new in v7)

Two bands, keyed by the capture path already chosen per figure:

- **Renders / composites** (`render.render`, `render.opengl`, Pillow composites): **200 to 800 KB.** Sub-200 KB on a render is a fault signal (blank frame, missing material, wrong engine) &#8212; investigate, don't pass.
- **UI-chrome** (`screenshot` / `screenshot_area` of Properties, node editor, Outliner, menus, popovers; warning vignettes): **no lower floor; accept ~10 to 250 KB.** Flag only if >250 KB (usually uncropped full-window leakage or needless upscaling) or <5 KB (truncated / near-empty). Never upscale a crisp flat-panel PNG to reach the render floor &#8212; it degrades it. Precedent: every UI-chrome capture L02 through L32 lands 10 to 210 KB (L06 icons 20 KB, L19 screenshots 12 to 42 KB, L31 panels 32 to 40 KB, L32 manual subpanels 122 to 208 KB).
...
## Color-dependent viewport renders (new in v6)

How to render a figure whose teaching point depends on a specific marker color (orange snap points, cyan edges, axis tints) reading correctly. These rules were discovered across Lesson 09 Phases 3 through 8 and govern every color-dependent BlenderMCP render in the course from here on. UI-chrome captures and plain-grey-geometry renders do NOT need them.

- **F12 `bpy.ops.render.render(write_still=True)`, NOT `bpy.ops.render.opengl`, for any color-dependent figure.** `render.opengl` renders the VIEWPORT SHADING (SOLID clay), so it returns flat grey clay on a washed light floor regardless of material / world / emission edits. A true F12 `render.render` respects materials, world darkness, and emission. The OpenGL viewport path stays fine for plain-grey-cube geometry figures where only silhouette / arrangement matters; switch to F12 the moment a marker color or emissive glow carries the message. (Lesson 09 Phase 3 Sub-chat B #7, confirmed across Phases 4 through 8.)
- **AgX is the real cause of emissive markers washing to cream / peach, not world brightness or emission strength.** The fix is `scene.view_settings.view_transform = 'Standard'` (plus `look = 'None'`) for any color-dependent marker render, then RESTORE to `'AgX'` at chat close. Under Standard, saturated orange and cyan read correctly even at modest emission. An early "drop the world to 0.05 + lower emission" workaround appeared to work only because dimming the world masked the AgX tone curve; Standard is the actual lever. (Lesson 09 Phase 3 Sub-chat A, superseding the Sub-chat B emission-only note.)
- **Working marker values under Standard on a 0.05 world** (the locked L09 recipe): world grey `0.05` strength `1.0`; `L09Orange` base + emission ≈ `(1.0, 0.30, 0.0)` / emission strength **1.2** (a 5.0 strength clips to white); `L09Cyan` base + emission ≈ `(0.16, 0.50, 1.0)` / emission strength **1.4** (a 5.0 base blows cyan to white); `L09GridLine` emission strength ≈ `0.9`. A brighter "active / focus" marker reads as pale yellow-white at base `(1.0, 0.52, 0.10)` / strength `2.2` to `3.2` and r slightly larger, which is the standard way to make one marker (active vertex, angle vertex) read distinct from its saturated-orange siblings. The dark 0.05 world also makes the dark floor plus emissive grid lines read crisply. RESTORE the documented base values (world 0.88 / str 1.0, `L09Orange` `(1.0,0.42,0.0)` str 5.0, `L09Cyan` `(0.29,0.62,1.0)` str 5.0, `L09GridLine` str 0.6) plus AgX at close.
- **Consistency across a phase:** if some figures in a phase render under AgX and others under Standard, the AgX ones read more washed (peach) and lighter-floored than the Standard ones. When a phase mixes them, review the set side by side and re-render the odd one for marker-color parity (Lesson 09 #7 was re-rendered under Standard to match its #4 / #5 / #6 / #8 siblings; the orange mean moved from a peach `(230,168,103)` into the sibling band `(252-254, 143-192, 13-33)`).

### Grid-floor as real line geometry (new in v6)

Blender's viewport grid does NOT appear in `render()` output, so any figure that needs a visible grid floor uses **real line geometry**, not the viewport grid:

- A dark floor plane (`L09FloorDark`, roughly `(0.13, 0.13, 0.14)` rough `0.9`) plus thin emissive quads as grid lines, built by a reusable `make_grid_lines(spacing)` builder: line width `max(spacing * 0.04, 0.012)`, lines in both directions across roughly a +/-12 extent, material `L09GridLine` (emissive, neutral grey emission color). Drive the spacing from the figure (1.0 standard / 0.1 fine / 10.0 coarse for a scale-comparison set).
- The builder pairs with the color-dependent render recipe above: the dark 0.05 world plus the emissive grid lines give the crisp dark-floor look used across all L09 render figures. (Lesson 09 Phase 2 grid figures; reused in Phases 3 through 8.)
- Helper-function namespaces do NOT persist across `blender:execute_blender_code` calls, so inline the helpers (`clear_demo` / `make_grid_lines` / `add_cube` / `world_to_camera_view` wrapper / render) into each render cell. `world_to_camera_view` needs a `Vector`, not a tuple.

### Synthetic-cage / overlay-over-render pattern (L07 / L08, formalized in v6)

When the bridge `screen.screenshot` cannot reach the overlay layer a figure needs (edit-mode dots, gizmo, a transform that only exists live), do NOT fight the capture path. Render a clean ghosted / SOLID base through the proven render path, record the projection pixels of the key points via `world_to_camera_view`, and draw the cage / markers / arrows as the INTEGRATION SVG overlay from those recorded pixels. The orange `#FF6B00` cage and markers live in the overlay, not in the render. This is the same family as the composite-overlay and small-frame multi-callout patterns, just driven by recorded projection pixels rather than by measuring the cropped PNG. (Lesson 08 c7 / c17 / c20 / c21 precedent; reused throughout L09 for the synthetic single-figure overlays such as #5 `c5`.)

### Local-tempdir screenshot carries overlays plus deterministic area crop (L08, formalized in v6)

A full-window `bpy.ops.screen.screenshot` to a local Windows tempdir DOES carry the on-screen panel / menu / popover overlays that a scoped `screenshot_area` or `render.opengl` would drop. So for docked-panel UI chrome, grab the whole window to the local tempdir, then Pillow-crop deterministically to the target area using `area.x / y / width / height` with the bottom-left to top-left origin flip (crop box `x -> x + width`, `y = WIN_H - (area.y + area.height) -> WIN_H - area.y`), then drop the top header-bar strip. Use `space.context = 'MODIFIER'` (or the needed enum) to switch the Properties tab before the grab, and `DRAW_WIN_SWAP` x3 to flush the matrix after any `view3d.view_axis` / `view_camera` / `view_selected` change. (Lesson 08 Phase 3; reused for the L09 clean-canvas ortho captures.)

### Ghost-blend luminance dependence (L08, formalized in v6)

The alpha of a BLEND "ghost" material that reads as a clear half-transparent ghost is render-luminance-dependent: roughly `0.25` alpha on a light-grey (≈0.88) world, roughly `0.45` on a darker world. When a figure switches to the 0.05 dark-world color-dependent recipe, the ghost needs a brighter / higher-alpha variant (the L09 `L09GhostB` at alpha ≈0.45) to stay legible against the dark floor. Pick the ghost alpha from the world the figure renders under, not from a single fixed value. (Lesson 08 Phase 4; reused for L09 axis-locking and relative-vs-absolute ghosts.)

### Edge-crease operator plus inset_individual supporting-loop generator (L08, formalized in v6)

There is no bmesh crease layer to set directly in this build; set edge crease via the `bpy.ops.transform.edge_crease` operator on the selected edges. To generate supporting / holding loops for a subdivision-surface demo without a full manual loop-cut pass, `bpy.ops.mesh.inset_individual` on the target faces produces a clean ring of supporting edges. (Lesson 08 Phase 5.)

### Spread-matched-threshold craft for parametric-comparison composites (L08, formalized in v6)

For a multi-panel composite that compares the same operator at different parameter values (subdivision levels, merge thresholds, array counts), choose the parameter spread so adjacent panels read DISTINCTLY at thumbnail scale, not just numerically. A linear value sweep often reads as near-identical neighbors; pick values whose visible result steps obviously between panels. Apply this before the composite step (cheap to recapture pre-composite). (Lesson 08 Phase 6.)

### Order-demo legibility (L08, formalized in v6)

When a figure demonstrates that operation ORDER matters (modifier stack order, transform order), pick an interacting pair whose two orderings produce visibly different results. A pair whose orderings happen to converge to near-identical geometry teaches nothing at thumbnail scale; choose the pair for maximum visible divergence between the two orders. (Lesson 08 Phase 7.)

## Phase 7 Mirror-modifier and X-Mirror 5.1.1 UI notes (new in v6)

Layout facts for symmetry-workflow UI-chrome figures in Blender 5.1.1 (resolved by reading the registered panel definitions, not by guessing menu layout):

- **The Mirror modifier panel exposes Axis as an X / Y / Z toggle-button ROW**, not three separate checkboxes; the enabled axis button reads blue. Clipping and Merge are checkboxes below the Mirror Object field. For a Mirror-setup figure, the three teaching callouts are the Axis X toggle, the Clipping checkbox, and the Merge checkbox. (Lesson 09 Phase 7 Sub-chat A #28.)
- **The Edit-Mode X-Mirror control is NOT a single "X-Mirror" checkbox.** It is the Mirror: X / Y / Z toggle-button row inside the Edit-Mode **Options** dropdown popover (Transform subsection), bound to `mesh.use_mirror_x / y / z`; the X button blue means enabled. The Options dropdown is transient, so route this capture to the Win+Shift+S overlay-freeze path. (Lesson 09 Phase 7 Sub-chat A #32.)

## Phase 8 clean-canvas viewport staging plus warning-vignette overlay host (new in v6)

### Clean-canvas viewport staging for ortho / reference captures

For clean orthographic-view or reference-image captures, hide the viewport chrome before the grab: `space.show_region_toolbar = False` (left T-toolbar), `space.show_region_ui = False` (N-panel), `space.show_gizmo_navigate = False` (nav gizmo); SOLID shading with `space.overlay.show_object_outline = True` for the orange selected-object outline; overlays floor + X/Y axes ON, cursor / text / stats OFF. Capture each view via `view3d.view_axis` (camera via `view_camera`) plus `view_selected`, with `DRAW_WIN_SWAP` x3 after every view change, then crop each to the clean canvas region (header strip dropped) and Pillow-composite the multi-view grid with `#FF6B00` 2px dividers. Restore the hidden regions at close. (Lesson 09 Phase 8 Sub-chat A #42 ortho 2x2 + #43 reference-image trace.)

### Pillow warning-vignette as overlay host

For a WARNING INFOGRAPHIC figure ("always apply scale" style: a title banner, several problem cards each with a red-X badge plus warning-triangle glyph, and a center green-check solution card), the figure is NOT a natural viewport render and NOT a Canva raster. Build it as a Pillow-composited vignette PNG that acts purely as an overlay HOST: house palette (warning-red `#E63946` + `#ffffff`, positive-green `#4ADE80`, orange `#FF6B00` accents, yellow `#F5C242` footer rail, dark-grey to warm-amber gradient bg), with CLEAR BANDS reserved where the INTEGRATION overlay text will land. NO baked text: every word (title, center command, the problem-scenario labels, the solution) is the `c{N}` overlay drawn at INTEGRATION over the recorded clear-band pixels. Compose tight bands (enlarge the center card so the command band is clear; drop dead leaders) and QA-view before disk-write. The same host pattern serves the #40 and #54 warning vignettes. (Lesson 09 Phase 8 #40; reused for Phase 10 #54.)

## Pixel-verify every PNG overlay before commit (L09, formalized in v6)

Before committing any PNG-overlay figure, verify the callout coordinates against the ACTUAL cropped PNG pixels, not against header-layout or popover-layout reasoning. Blender 5.1.1 control order is not always what a spec implies (the L09 #1 header order is Global / pivot / magnet / snap-dropdown, with the snapping controls NOT leftmost). Method:

- Pillow-scan the cropped PNG for the highlighted / active control (or the divider positions on a composite), then draw the overlay at those measured coordinates over the PNG and view-verify the composite BEFORE the `edit_file` commit.
- **cairosvg IS available on Claude's side**, so when the verification work runs Claude-side the overlay-over-PNG render-check can be a 1:1 faithful cairosvg rasterization of the exact overlay SVG composited over the PNG (the overlay viewBox equals the PNG pixel dimensions, so coordinates map 1:1). This is the preferred high-fidelity check and was used for the L09 Phase 10 #54 warning-vignette verification.
- **cairosvg is NOT available inside Blender 5.1 Python on Windows** (no native libcairo DLL; `pip install cairosvg` installs to user-site but import fails). When the verification must run bridge-side / Blender-side, substitute an equivalent PIL/Pillow overlay-composite drawn at the exact viewBox coordinates plus a Claude view-verify (since viewBox equals PNG pixels, the Pillow composite is also 1:1 faithful). This was the substitute used at L09 Phase 8 INTEGRATION when cairosvg was unavailable in the Blender Python that ran the check.

- **Bridge-side Pillow in-place crop (UI-chrome capture win, new in v7 / L18 P6).** For UI-chrome PNG figures (Properties tabs, node editors), skip the base64 round-trip entirely. Capture the raw `screenshot_area` to the Windows tempdir, `shutil.copyfile` it to the WSL `images\` dir, then crop **in place with bridge-side Pillow** (`PIL 12.2.0` is available inside Blender 5.1.1 Python on Windows) straight to the deliverable path: `Image.open(raw).crop(bounds).save(deliverable)`. This is faster than relaying a ~100k-char base64 payload Claude-side and cropping there. The cairosvg overlay pixel-verify is still run Claude-side (copy the cropped deliverable via `copy_file_user_to_claude`, composite the c{N} overlay over it at viewBox=crop-px, view to confirm 1:1). Proven across L18 #15 (Volume probe), #16 (Sphere probe), and #08 (Compositor Glare node).
- **Maximize-stall fix (L18 P6).** When capturing a maximized Properties/Node-editor area, isolate each step in its OWN `blender:execute_blender_code` call: (1) set the tab context / `tree_type`; (2) `screen_full_area()` maximize; (3) `screenshot_area`; (4) `screen_full_area()` restore. NEVER combine the maximize with a context-change redraw in a single call — that combination hung the bridge mid-capture (L18 #15, two sessions). Re-find the area object after maximizing (maximize swaps to a new fullscreen screen). Maximize yields a 1920x976 capture area.
- **5.1.1 Compositor is node-group-based (L18 #08).** `scene.use_nodes` + `scene.node_tree` are GONE; the compositor lives in `scene.compositing_node_group` (a `CompositorNodeTree` datablock — create with `bpy.data.node_groups.new(name, "CompositorNodeTree")` and assign). There is **no `CompositorNodeComposite` output node**; use a `NodeGroupOutput` node with an Image output socket added via `ng.interface.new_socket("Image", in_out='OUTPUT', socket_type='NodeSocketColor')`. The **Glare** node is fully socket-driven: `glare_type` is no longer an RNA enum — set the **Type** MENU input socket instead (`glare.inputs["Type"].default_value = "Bloom"`; valid: Bloom / Ghosts / Streaks / Fog Glow / Simple Star / Sun Beams / Kernel). Threshold/Strength/Size/Saturation/Tint are likewise input sockets. To capture the compositor, switch an area to `NODE_EDITOR` with `space.tree_type='CompositorNodeTree'`, `bpy.ops.node.view_all()` (inside a WINDOW-region temp_override) to frame, then run the isolated maximize/capture/restore sequence.
...
- **Boolean solver enum in 5.1.1 is `FLOAT` / `EXACT` / `MANIFOLD` &#8212; there is NO `'FAST'` value.** Legacy / pre-5.x "Fast" maps to `FLOAT`; "Exact" stays `EXACT`. Setting `boolean_mod.solver = 'FAST'` raises an enum error. For a Fast-vs-Exact teaching figure, stage `FLOAT` (the artifact-prone path: non-manifold edges, z-fight striping, torn rims on double-coincident bores) against `EXACT` (clean). Specs and AI prompts written from older Blender docs still say "Fast"; translate to `FLOAT`. (Lesson 31 #38 Boolean solver comparison.)
- **`r3d.view_perspective = 'CAMERA'` (idempotent) to enter camera view before `render.opengl`, NOT `bpy.ops.view3d.view_camera()`.** The operator TOGGLES camera view, so if the viewport is already in camera view it EXITS and the subsequent `render.opengl(view_context=True)` comes out blank. Assigning the `view_perspective` enum is safe to call repeatedly and always lands in camera view. (Lesson 31 #36 first render blanked by the toggle before this fix.)
- **`render.opengl(view_context=True)` inherits the viewport camera-view `view_camera_zoom` / `view_camera_offset`** from whichever area/region the render override targets; a stale non-zero zoom/offset throws the subject off-frame (typically into a corner) even when the scene-camera `world_to_camera_view` NDC check reads centered. Fix: after entering camera view, reset `region_3d.view_camera_zoom = 0` and `region_3d.view_camera_offset = (0.0, 0.0)` ON THE SAME area / region / `region_3d` that the `render.opengl` override uses, before rendering. Resetting a different region's r3d does nothing. (Lesson 31 #4 P3 off-frame render.)
...
17. **Color-dependent render recipe (new in v6):** Any figure whose teaching point depends on a marker color renders via F12 `bpy.ops.render.render(write_still=True)` (NOT `render.opengl`), under `view_transform='Standard'` + `look='None'`, on a 0.05 world, with the locked L09Orange `(1.0,0.30,0.0)`/emis 1.2 and L09Cyan `(0.16,0.50,1.0)`/emis 1.4 marker values (a bright active marker at `(1.0,0.52,0.10)`/emis 2.2 to 3.2). RESTORE AgX + world 0.88 + base material values at chat close. Plain-grey geometry and UI-chrome captures are exempt.
18. **Grid-floor as real line geometry (new in v6):** The viewport grid does not render, so grid figures build a dark floor plane plus thin emissive grid-line quads via the `make_grid_lines(spacing)` builder. Inline all render helpers per call (namespaces do not persist across `blender:execute_blender_code`).
19. **Pixel-verify every PNG overlay before commit (new in v6):** Verify callout coordinates against the actual cropped PNG pixels (Pillow scan plus an overlay-over-PNG composite), never by header/popover-layout reasoning, BEFORE the `edit_file` commit. cairosvg gives a 1:1 faithful render-check on Claude's side; substitute an equivalent PIL/Pillow composite when the check must run bridge-side (cairosvg is unavailable inside Blender 5.1 Python on Windows).
20. **Capture-path routing (new in v6):** Render-API-unreachable overlays use the synthetic-cage / overlay-over-render pattern (recorded `world_to_camera_view` pixels drive an INTEGRATION SVG overlay over a clean render). Docked-panel UI chrome uses a full-window local-tempdir `screen.screenshot` (which carries the panel overlays) plus a deterministic area crop. Transient popovers (and any panel a left-open popover occludes, which the bridge cannot dismiss) route to the Win+Shift+S overlay-freeze path. Clean ortho / reference captures hide toolbar + N-panel + nav gizmo first.
21. **Warning-vignette as overlay host (new in v6):** Warning-infographic figures are Pillow-composited vignette PNGs with NO baked text, reserving clear bands for the `c{N}` INTEGRATION overlay (warning-red problems, positive-green solution). Not a render, not a Canva raster.
22. **PNG accept-size band (new in v7):** Renders / composites 200 to 800 KB (sub-200 KB = fault). UI-chrome captures (Properties / node editor / Outliner / menus / popovers / warning vignettes) have no floor: accept ~10 to 250 KB, flag only >250 KB or <5 KB; never upscale a flat panel to reach the render floor. Class keyed by capture path.

---

## Revision log

- **v8 (current, 2026-06-27).** Compaction-only pass: collapsed the bloated `Status:` header (was a ~450-word re-narration of every v4-v7 addition) to two lines pointing at the body + Locked decisions; trimmed each v2-v7 revision-log entry to a single line. No rule, value, section, or Locked decision changed; body sections and the 22 Locked decisions are untouched. Content-identical to v7 in every operative respect.
- **v7 (2026-06-27).** Added the PNG accept-size band (Capture workflow): renders / composites 200 to 800 KB (sub-200 KB = fault); UI-chrome no floor, accept ~10 to 250 KB, flag only >250 KB or <5 KB, never upscale flat panels. Locked decisions 21 -> 22. No PNG re-issue.
- **v6 (2026-06-02).** Folded in live L07/L08/L09 capture + render craft: color-dependent render recipe (F12 `render.render`, AgX->Standard, L09Orange/L09Cyan values on a 0.05 world), grid-floor real line geometry, synthetic-cage overlay-over-render, local-tempdir screenshot + deterministic crop, ghost-blend luminance, edge-crease + inset_individual loops, spread-matched-threshold, order-demo legibility, Phase 7 Mirror/X-Mirror 5.1.1 UI, Phase 8 clean-canvas staging + warning-vignette overlay host, pixel-verify-before-commit (cairosvg Claude-side / PIL bridge-side). Locked decisions 16 -> 21. No re-issue.
- **v5 (2026-05-25).** Added the Capture workflow section (add-on deps, BlenderMCP capture paths + UNC truncation workaround, Face Orientation recipe, manual capture cadence, modal capture criterion, SOLID-shading readability, multi-panel composite QA), the Blender 5.1.1 op-name discoveries section, and the cross-sub-chat composite bullet. Locked decisions 13 -> 16. No re-issue.
- **v4 (2026-05-21).** Added overlay badge colorway split (positive green / warning red), em-dash discipline, inline-stacked PNG annotation overlay pattern, 800x680 dense viewBox variant, HTML integration patterns + figure-indent depth rules, SVG inlining checklist, footer-tip left-rail Variant B. Locked decisions 7 -> 13. No re-issue.
- **v3 (2026-05-20).** Added `#E63946` X-axis red to the palette, documented dual roles for `#4ADE80` / `#4A9EFF`, the axis color convention (X=red/Y=green/Z=blue), the orange tie-break rule, and the cheat-sheet badge filled-vs-muted policy.
- **v2.** Switched from self-contained dark panel to transparent / page-inheriting `currentColor` (fallback `color="#222"`); added course-specific data section with real module names; pilot redone as `lesson_01_08_folder_structure.svg`.
- **v1.** Initial pilot, dark self-contained panel theme. Superseded.
